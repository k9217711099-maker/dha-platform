...
const PRESETS: { name: string; desc: string; rows: Row[] }[] = [
  ...
  {
    name: 'Инспекция номера (полный осмотр)',
    desc: 'Технический осмотр номера по зонам',
    rows: [
      R('HEADER', 'Вход'),
      R('ITEM', 'Дверь / дверная коробка / наличник в хорошем состоянии'),
      R('ITEM', 'Доводчик / пружинные петли исправны, дверь закрывается бесшумно'),
      R('ITEM', 'Ограничители открывания и уплотнители прочно закреплены'),
      R('ITEM', 'Порог в хорошем состоянии'),
      R('ITEM', 'Номер комнаты / знак запрета курения в наличии'),
      R('ITEM', 'Электронный замок исправен (при необходимости заменить батарейки)'),
      R('ITEM', 'Задвижка исправна'),
      R('ITEM', 'Предохранительная защёлка / глазок исправны'),
      R('ITEM', 'План эвакуации / указания по ТБ в наличии и актуальны', { requirePhoto: true }),
      R('ITEM', 'Сигнализатор дыма исправен; головки спринклеров не загорожены'),
      R('HEADER', 'Коридор'),
      R('ITEM', 'Дверь / коробка / ручка в хорошем состоянии, закрывается надлежаще'),
      R('ITEM', 'Напольное покрытие / стены / потолок без пятен и следов износа'),
      R('ITEM', 'Полки / вешалка в хорошем состоянии'),
      R('ITEM', 'Утюг / гладильная доска исправны'),
      R('ITEM', 'Мини-бар / холодильник исправен'),
      R('ITEM', 'Сейф исправен'),
      R('HEADER', 'Стены / потолок / пол'),
      R('ITEM', 'Стены без пятен и следов износа'),
      R('ITEM', 'Потолок / съёмные панели в хорошем состоянии'),
      R('ITEM', 'Нет признаков повреждений от воды или утечек'),
      R('ITEM', 'Межкомнатная дверь / фурнитура; закрывается бесшумно'),
      R('ITEM', 'Напольное покрытие в хорошем состоянии'),
      R('ITEM', 'Плинтусы прочно закреплены'),
      R('ITEM', 'Зеркала / картины прочно закреплены'),
      R('HEADER', 'Электронные устройства'),
      R('ITEM', 'Телевизор исправен, качество приёма и изображения'),
      R('ITEM', 'Поворотная подставка ТВ исправна и закреплена'),
      R('ITEM', 'Пульт ДУ исправен (ежегодная замена батареек)'),
      R('ITEM', 'Кабели зафиксированы, опрятный вид'),
      R('ITEM', 'Будильник с док-станцией исправен, время верное'),
      R('ITEM', 'Кофемашина исправна'),
      R('HEADER', 'Телефоны'),
      R('ITEM', 'Трубки / лицевые панели / шнуры в хорошем состоянии'),
      R('ITEM', 'Тональные сигналы и кнопки быстрого набора работают'),
      R('ITEM', 'Голосовая почта и световой индикатор исправны'),
      R('HEADER', 'Освещение и электрика'),
      R('ITEM', 'Все светильники, лампы и реостаты исправны'),
      R('ITEM', 'Панели розеток и выключателей в хорошем состоянии'),
      R('ITEM', 'Штепсельные вилки без повреждений оболочек / оголённых проводов'),
      R('ITEM', 'Все лампы исправны и подобраны по размеру'),
      R('HEADER', 'Оконная зона'),
      R('ITEM', 'Шторы / занавески / тюль легко двигаются вдоль карнизов'),
      R('ITEM', 'Окно / балконная дверь / замки закрываются надлежаще'),
      R('ITEM', 'Нет признаков повреждений от воды'),
      R('ITEM', 'Пол / стены / навес / мебель на балконе в хорошем состоянии'),
      R('HEADER', 'ОВК (отопление, вентиляция, кондиционирование)'),
      R('ITEM', 'Регулировка температуры и термостат исправны'),
      R('ITEM', 'Нет протечек, отвод конденсата в норме'),
      R('ITEM', 'Решётки / фильтры / теплообменники / поддон очищены'),
      R('ITEM', 'Вентиляторный электродвигатель и проводка исправны'),
      R('ITEM', 'Уровень шума не превышает допустимый'),
      R('HEADER', 'Мебель'),
      R('ITEM', 'Рабочий / письменный стол в хорошем состоянии'),
      R('ITEM', 'Стулья (деревянные части и обивка)'),
      R('ITEM', 'Комоды / ящики / прикроватные тумбочки'),
      R('ITEM', 'Остов и изголовье кровати'),
      R('ITEM', 'Пружинная сетка и матрас'),
      R('ITEM', 'Диван-кровать легко раскладывается'),
      R('ITEM', 'Журнальный и туалетный столики'),
      R('ITEM', 'Подставка для багажа'),
      R('HEADER', 'Ванная: дверь и пол'),
      R('ITEM', 'Дверь / коробка / наличник в хорошем состоянии'),
      R('ITEM', 'Фурнитура / замок / ограничитель; закрывается надлежаще'),
      R('ITEM', 'Пол без разломанных или расколотых плиток'),
      R('ITEM', 'Затирка швов без повреждений'),
      R('HEADER', 'Умывальная зона и раковина'),
      R('ITEM', 'Уплотнение столешницы / спинки умывальника'),
      R('ITEM', 'Запорный кран исправен'),
      R('ITEM', 'Температура горячей воды ≤ 49 °C, напор в норме'),
      R('ITEM', 'Туалетный шкаф / дверцы / ящики закрываются надлежаще'),
      R('ITEM', 'Осветительная арматура с исправными лампами'),
      R('ITEM', 'Туалетное зеркало закреплено'),
      R('ITEM', 'Краны без подтекания'),
      R('ITEM', 'Слив и сливная пробка исправны'),
      R('ITEM', 'Стены / потолки без пятен, трещин, отколотых плиток'),
      R('HEADER', 'Туалетная зона'),
      R('ITEM', 'Унитаз и сливной рычаг исправны'),
      R('ITEM', 'Сиденье унитаза закреплено'),
      R('ITEM', 'Запорные вентили исправны'),
      R('ITEM', 'Унитаз прикреплён к полу и загерметизирован'),
    ],
  },
];
