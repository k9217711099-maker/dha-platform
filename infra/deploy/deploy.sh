...
# 4. Сборка НАЧИСТО. Кэш turbo/Next уже вычищен в deploy.yml ДО этого скрипта (там нет
# лага самообновления). TURBO_FORCE + rm .next — подстраховка, чтобы точно ничего не
# «реплеилось» из кэша: turbo на этом сервере давал ложные FULL-TURBO cache-hit и
# восстанавливал старый бандл (прод замерзал на старом коде при зелёном деплое). Реальная
# полная сборка ~2–3 мин (admin ~38с, web ~22с, api секунды) — быстро, инкремент не нужен.
