...
# 4. Сборка — ВАРИАНТ 1: инкрементально (turbo собирает только изменённые пакеты).
# НЕ форсим и НЕ чистим кэш: пересобирается лишь то, что реально поменялось (правка
# только в API → web/admin берутся из turbo-кэша за секунды). Старый bug «кэш отдаёт
# старый бандл» закрыт тем, что NEXT_PUBLIC_API_URL объявлен в turbo.json → входит в
# ключ кэша, смена адреса сама инвалидирует бандл. Порядок пакеты→apps turbo держит.
#
# API собираем ПРИНУДИТЕЛЬНО (--force): nest build одного пакета — секунды, зато dist/main.js
# гарантированно свежий. Без этого «залипший» кэш turbo (после аварийных сборок в шторме)
# отдавал старый бандл → деплой зелёный, а на проде старый код. Для web холодная пересборка
# на слабом VPS опасна (OOM) — её оставляем инкрементальной, форсим только дешёвый API.
pnpm exec turbo run build --filter=@dha/api --force
pnpm build
